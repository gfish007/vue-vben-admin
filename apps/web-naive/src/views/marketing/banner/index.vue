...
import type { BannerApi } from '#/api/marketing/banner.types';
...
// 查询条件
const queryForm = reactive<BannerApi.QueryParams['queryBody']>({
  title: undefined,
...
// 表格数据
const tableData = ref<BannerApi.BannerRecord[]>([]);
...
const editingRecord = ref<BannerApi.BannerSaveReq & { regionId?: number }>({
  title: '',
  imageUrl: '',
  jumpType: 0,
  jumpUrl: '',
...
const fetchData = async () => {
  ...
  try {
    ...
    const result = await queryBannerList({
      page: {
...
const columns = [
  ...
  {
    key: 'imageUrl',
    render: (row: BannerApi.BannerRecord) => {
      return h('img', {
        src: row.imageUrl,
        style: {
          width: '80px',
...
// 修改 handleEdit 函数
const handleEdit = async (row: BannerApi.BannerRecord) => {
  editLoading.value = true;
  try {
    modalTitle.value = '编辑Banner';
...
// 修改 handleDelete 函数
const handleDelete = async (row: BannerApi.BannerRecord) => {
  // 添加二次确认
  dialog.warning({
    title: '确认删除',
...
// 处理启用/禁用
const handleToggleStatus = async (row: BannerApi.BannerRecord) => {
  try {
...
const handleSave = async () => {
  ...
  try {
    ...
    await saveOrUpdateBanner(saveData);
...
<template>
  <Page description="管理系统中的Banner信息" title="Banner管理">
      ...
      <NCard class="query-card">
        <NForm :model="queryForm" inline>
          <div style="display: flex; flex-wrap: wrap; gap: 16px 24px; width: 100%; align-items: center;">
            ...
            <NFormItem label="区域" label-placement="left" style="margin-bottom: 0;">
              <NSelect
            ...
            <div style="display: flex; gap: 16px; margin-left: auto;">
              ...
              <NButton type="success" @click="handleAdd">新增Banner</NButton>
            </div>
          </div>
        </NForm>
      </NCard>
