...
import type { ServerApi } from '#/api/core/server.types';
...
const [Modal, modalApi] = useVbenModal({
  connectedComponent: ServerModal,
...
// 查询条件
const queryForm = reactive<ServerApi.QueryParams['queryBody']>({
  auditStatus: null,
  publishStatus: null,
  regionId: undefined,
  title: '', // 添加标题查询条件
...
// 表格数据
const tableData = ref<ServerApi.ServerRecord[]>([]);
...
const fetchData = async () => {
  loading.value = true;
  try {
    const result = await queryServerList({
      page: {
...
// 处理编辑
const handleEdit = (row: ServerApi.ServerRecord) => {
  modalApi.setData({
    id: row.id,
    title: '编辑服务',
  });
...
<template>
  <Page description="管理系统中的地区服务信息" title="地区服务管理">
